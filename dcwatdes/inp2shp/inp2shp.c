...
main (int argc, char **argv)
{
  ...
  if ((argc != 9) ||
    ...
    {
      printf
	("inp2shp 0.2.2 (c) 2002, 2005, 2006 DC Water and Environment\n");
 ...
 */
void
exit_inp2shp (int error)
{
  if (hPipeSHP)
  {
               SHPWriteHeader(hPipeSHP);
    SHPClose (hPipeSHP);
}
  if (hPipeDBF)
    DBFClose (hPipeDBF);
  ...
  if (hJunctionSHP) {
                    SHPWriteHeader(hJunctionSHP);
    SHPClose (hJunctionSHP);
}
  if (hTankSHP) {
                SHPWriteHeader(hTankSHP);
    SHPClose (hTankSHP);
}
  if (hTankDBF)
    DBFClose (hTankDBF);
  if (hReservoirSHP) {
                     SHPWriteHeader(hReservoirSHP);
    SHPClose (hReservoirSHP);
}
  if (hReservoirDBF)
    DBFClose (hReservoirDBF);
  if (hPumpSHP) {
                SHPWriteHeader(hPumpSHP);
    SHPClose (hPumpSHP);
}
  if (hPumpDBF)
    DBFClose (hPumpDBF);
  if (hValveSHP) {
                 SHPWriteHeader(hValveSHP);
    SHPClose (hValveSHP);
}
  if (hValveDBF)
    DBFClose (hValveDBF);
...
write_node ()
{
  ...
  switch (nodetype)
    ...
    default:
            fprintf(stderr,
            "FATAL ERROR: Unknown node type \"%d\".\n", 
            nodetype);
            exit_inp2shp (1);
    break;  
    }
...
write_vertex ()
{
  ...
  if (strcmp (vertex_line_name, Tok[0]) != 0)
	...
	{
	  error = ENgetlinktype (vertex_line_index, &linktype);
	  if (0 != error)
	    {
	      fprintf (stderr,
...
write_reservoir (int index)
{
 ...
 error = ENgetnodevalue (index, EN_PATTERN, &f);
  ...
  if (0 != error)
   ...
   error =
    DBFWriteStringAttribute (hReservoirDBF, num_reservoirs, 10, string);
#ifdef DEBUG
  fprintf (stderr, "DBFWriteStringAttribute(%d, 10, \"%s\") returned %d in write_reservoir()\n",
	   num_junctions, string, returnvalue);
#endif
}                                         					 
  num_reservoirs++;
 ...
 */
int
find_from_node (int nodeindex, char *nodeid)
{
  int i = 1;
  int from_node;
  int to_node;
...
write_pipe_shape ()
{
  ...
  while (((string == NULL) || (strcmp (string, vertex_line_name) != 0))
	 && (index >= -1));
  if (index != -2)
    {
 ...
 */
int
create_tank_shapefile (char *filename)
{
 ...
 */
int
create_reservoir_shapefile (char *filename)
{
 ...
 */
int
create_pump_shapefile (char *filename)
{
 ...
 */
int
create_valve_shapefile (char *filename)
{
 ...
 */
int
create_pipe_shapefile (char *filename)
{
